...
const DeleteConfirmationModal = ({ item, onClose, onConfirm, isDeleting = false }) => {
  if (!item) return null;

  // Check if this is a bulk action (item will have a count property or be an array)
  const isBulkAction = Array.isArray(item) || item.count !== undefined;
  const itemCount = Array.isArray(item) ? item.length : (item.count || 1);
        ...
        <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 dark:bg-dark-bg-secondary">
          <div className="sm:flex sm:items-start">
            ...
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-dark-text-primary">
                {isBulkAction ? "Delete Items" : "Delete Item"}
              </h3>
              <div className="mt-2">
                {isBulkAction ? (
                  <p className="text-sm text-gray-500 dark:text-dark-text-secondary">
                    Are you sure you want to delete <span className="font-medium">{itemCount} items</span>? 
                    This action will move them to the Recycle Bin.
                  </p>
                ) : (
                  <p className="text-sm text-gray-500 dark:text-dark-text-secondary">
          ...
          <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
            <LoadingButton
              ...
              onClick={onConfirm}
            >
              {isBulkAction ? "Delete All" : "Delete"}
            </LoadingButton>
            <button
              type="button"
