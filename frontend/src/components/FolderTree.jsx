...
const FolderItem = ({ folder, selectedFolder, onSelectFolder, level = 0, expandedFolders, onToggleFolder }) => {
  const isSelected = folder.id === selectedFolder;
  const isExpanded = expandedFolders ? expandedFolders.has(folder.id) : true;
...
const FolderTree = ({ folders, selectedFolder, onSelectFolder, expandedFolders, onToggleFolder }) => {
  return (
    <div className="py-2">
      {folders.map((folder) => (
        <FolderItem
