...
import MobileFileCard from './MobileFileCard'; // Import mobile file card component
...
const DocumentList = ({ 
  ...
  const renderListView = () => (
    <div className="h-full flex flex-col">
      {/* Mobile view - use MobileFileCard for better mobile experience */}
      ...
      {/* Desktop view - traditional table view */}
      <div className="hidden md:flex md:flex-col h-full">
        ...
        <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-gray-50 border-b border-gray-200 text-xs font-medium text-gray-500 uppercase tracking-wider dark:bg-dark-bg-tertiary dark:border-dark-border dark:text-dark-text-secondary">
          <div className="col-span-5 flex items-center">
            <input
              type="checkbox"
              className="h-4 w-4 rounded border-gray-300 text-gray-700 focus:ring-gray-500 dark:bg-dark-bg-secondary dark:border-dark-border"
          ...
          {sortedItems.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-200 dark:divide-dark-border">
              {sortedItems.map((item) => (
                <li
                  key={item.id}
                  className={`grid grid-cols-12 gap-4 px-6 py-4 hover:bg-gray-50 cursor-pointer dark:hover:bg-dark-bg-primary border-l-4 transition-all duration-200 ${
                    selectedItems.some(selectedItem => selectedItem.id === item.id)
                      ? 'bg-gray-100 dark:bg-dark-bg-tertiary border-gray-400 dark:border-gray-500' : 'border-transparent'
                  }`}
                  onClick={(e) => {
                    if (e.target.type !== 'checkbox') {
                      if (item.type === 'folder') {
                        onSelectFolder(item);
                        return; // Don't select when opening a folder
                      } else {
  ...
  const renderThumbnailView = () => (
    <div className="h-full flex flex-col">
      {/* Mobile and Desktop - unified thumbnail grid view */}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {sortedItems.map((item) => (
              <Thumbnail
                ...
                onSelect={onItemSelect}
                isSelected={selectedItems.some(selectedItem => selectedItem.id === item.id)}
                isOpen={item.type === 'file' && selectedDocument && selectedDocument.id === item.id}
              />
            ))}
          </div>
        )}
  ...
  return (
    <div 
      ...
      onContextMenu={(e) => {
        ...
        // Call the onContextMenuAction prop to handle right-click on empty area
        if (onContextMenuAction) {
          onContextMenuAction(e, { type: 'empty-area', name: 'Empty Area', folderId: selectedFolder });
        }
      }}
    >
