...
const NewFolderModal = ({ folders, selectedFolder, onClose, onCreate, isLoading = false }) => {
  const [folderName, setFolderName] = useState('');
  const [destinationFolder, setDestinationFolder] = useState(selectedFolder || null); // Default to currently selected folder or root if none selected
  ...
  const handleCreate = () => {
    ...
    onCreate(
      folderName,
      destinationFolder // This will be the actual folder ID or null for root
    );
  ...
  const getFolderName = (folderId) => {
    if (!folderId) return 'Root'; // Handle null parent ID
  ...
  const currentPath = destinationFolder ? getFolderName(destinationFolder) || 'Unknown' : 'Root';
                    ...
                    <div>
                      ...
                      <div className="mt-1">
                        <CustomSelect
                          ...
                          options={[{ value: null, label: 'Root Folder' }, ...getAllFoldersWithPaths(folders).map((f) => ({
                            value: f.id,
                            label: f.name
                          }))]}
