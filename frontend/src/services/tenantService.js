 ...
 */
export const getTenantBillingInfo = async () => {
  try {
    const response = await api.get('/tenants/billing');
...
/**
 * Get personalized billing plans for the current tenant
 * @returns {Promise<Object>} Personalized billing plans with custom pricing
 */
export const getPersonalizedBillingPlans = async () => {
  try {
    const response = await api.get('/tenants/billing/plans');
    return response.data.data;
  } catch (error) {
    throw new Error(error.response?.data?.message || 'Failed to fetch personalized billing plans');
  }
};
 ...
 */
export const updateTenantPlan = async (planId) => {
  try {
    const response = await api.patch('/tenants/plan', { subscriptionPlanId: planId });
    return response.data.data;
  } catch (error) {
    throw new Error(error.response?.data?.message || 'Failed to update tenant plan');
  }
};