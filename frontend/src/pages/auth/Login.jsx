...
const Login = () => {
  ...
  const loginMutation = useMutation({
    mutationFn: ({ email, password, rememberMe }) => login(email, password, rememberMe),
    onSuccess: (data) => {
      if (data.success) {
        // Show success toast
  ...
  // Copy to clipboard function with secure context handling
  const copyToClipboard = async (text) => {
    try {
      ...
      // Fallback to showing an error toast if copy fails
