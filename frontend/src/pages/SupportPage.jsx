...
const SupportPage = () => {
  ...
  const { data: tickets = [], isLoading: isTicketsLoading } = useQuery({
    queryKey: ['supportTickets', filters, sortBy, isAdmin],
    queryFn: () => {
      if (isAdmin) {
        return supportTicketService.getAllTickets(filters);
      } else {
        return supportTicketService.getTenantTickets(filters);
      }
    },
  ...
  const filteredAndSortedTickets = tickets
    .filter(ticket => {
      ...
      if (!isAdmin && ticket.userId !== userInfo?.id) return false; // Show only user's own tickets
      return true;
    })
    .sort((a, b) => {
      if (sortBy === 'newest') {
  ...
  const createTicketMutation = useMutation({
    mutationFn: (ticketData) => supportTicketService.createTicket(ticketData),
    onSuccess: () => {
  ...
  const handleCreateTicket = (e) => {
    ...
    const ticketData = {
      title,
      description,
      attachments: attachments.map(file => file.name), // Just send filename for now
    };
  ...
  const addCommentMutation = useMutation({
    mutationFn: ({ ticketId, commentData }) => supportTicketService.addComment(ticketId, commentData),
    onSuccess: (response, variables) => {
      queryClient.invalidateQueries(['supportTickets']);
      // Update the selected ticket with the new comment
      const newCommentObj = {
        ...variables.commentData,
        id: response.id || `cmt_${Date.now()}`,
        timestamp: response.createdAt || new Date().toISOString(),
        author: userInfo?.name || 'Current User',
  ...
  const handleAddComment = () => {
    ...
    addCommentMutation.mutate({ ticketId: selectedTicket.id, commentData });
  };

  // Mutation for updating ticket status
  const updateTicketMutation = useMutation({
    mutationFn: ({ ticketId, updateData }) => supportTicketService.updateTicket(ticketId, updateData),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries(['supportTickets']);
      // Update the selected ticket status
      if (selectedTicket && selectedTicket.id === variables.ticketId) {
        setSelectedTicket({ ...selectedTicket, status: variables.updateData.status });
      }
      toast.success(`Ticket status updated to ${variables.updateData.status}`);
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Failed to update ticket status. Please try again.');
    },
  });
  ...
  const handleStatusChange = (ticketId, newStatus) => {
    ...
    updateTicketMutation.mutate({ ticketId, updateData });
  };
          ...
          <div className="overflow-y-auto h-full">
            {isTicketsLoading ? (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="mt-4 text-sm text-gray-500 dark:text-dark-text-secondary">Loading tickets...</p>
              </div>
                    ...
                    <div className="mt-4 border-b border-gray-200 dark:border-dark-border">
                      ...
                      <div className="mt-2 flex items-center">
                        <select
                          ...
                          className={`block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-accent focus:border-accent sm:text-sm rounded-md dark:bg-dark-bg-tertiary dark:border-dark-border dark:text-dark-text-primary ${updateTicketMutation.isPending ? 'opacity-50' : ''}`}
                        >
                          <option value="Open">Open</option>
                          <option value="In Progress">In Progress</option>
                          <option value="Resolved">Resolved</option>
