...
const ProfilePage = () => {
  ...
  // Dummy data for roles and permissions (fallback if not in user object)
  const fallbackRolesAndPermissions = {
    roles: [
      { id: 'r1', name: 'User', description: 'Standard user access' }
    ],
    permissions: [
      { id: 'p1', name: 'View Documents', description: 'Ability to view documents' }
    ]
  };

  // Get real roles and permissions from user object
  const userRoles = user.userRoles?.map(ur => ({
    id: ur.role?.id,
    name: ur.role?.name,
    description: ur.role?.description
  })) || fallbackRolesAndPermissions.roles;

  const userPermissions = user.userRoles?.reduce((acc, ur) => {
    const rolePermissions = ur.role?.permissions?.map(rp => ({
      id: rp.permission?.id,
    ...
    return acc;
  }, []) || fallbackRolesAndPermissions.permissions;
  ...
  const handleImageChange = (e) => {
    ...
    if (!validTypes.includes(file.type)) {
      ...
      showErrorToast('Invalid file format. Please upload JPG, PNG, or WebP images.');
      return;
    }

    // Validate file size (e.g., max 5MB)
    if (file.size > 5 * 1024 * 1024) {
      showErrorToast('File size too large. Maximum size is 5MB.');
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsUpdating(false);
      showErrorToast('Failed to update profile picture. Please try again.');
    }
  };
  ...
  const handleSaveContactInfo = async () => {
    try {
      ...
      setIsUpdating(false);
      showErrorToast('Failed to update contact information. Please try again.');
    }
  };
        ...
        <div className="mt-6">
          ...
          {activeTab === 'security' && (
            <div className="max-w-md mx-auto">
              <h3 className="text-lg font-medium text-gray-900 dark:text-dark-text-primary mb-6">Change Password</h3>
              <form onSubmit={handleChangePasswordSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="mt-1 block w-full text-gray-900 dark:text-dark-text-primary bg-white border border-gray-300 rounded-md shadow-sm px-3 py-2 dark:bg-dark-bg-secondary dark:border-dark-border focus:ring-accent focus:border-accent"
                  />
                </div>
                ...
                <div className="pt-4 flex justify-end">
                  <LoadingButton
                    ...
                    className="px-4 py-2 rounded-md text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent bg-accent hover:bg-accent-dark dark:bg-accent-dark"
                  >
                    Change Password
                  </LoadingButton>
                </div>
